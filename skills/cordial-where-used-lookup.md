---
name: where-used-lookup
description: Reverse-dependency lookup — find every place a given sculpt block, HTML include, contact attribute, coupon code, or image is referenced across the account (which messages, orchestrations, templates, includes) so a marketer can safely edit or retire it without breaking sends. Use when a marketer/CSM/Solutions person types "where is this block/include used", "what uses this sculpt block/template", "where is this coupon code used", "which messages/orchestrations reference this attribute", "where is {$contact.attribute} used", "what drives the footer / which include builds X", "find everywhere this content key is referenced", or "what will break if I change/delete this block/include".
---

# Where-Used Lookup

Build a reverse-dependency report for a single target — sculpt block, HTML include, contact attribute, coupon code/string, or image — listing every asset (message, orchestration, template, include) that references it, so the marketer knows exactly what breaks before they edit or retire it. There is **no native where-used endpoint**; this is enumerate-and-scan.

> Shared mechanics live in the project-root `_reference/` folder (sibling to `recipes/`, i.e. `reference/` from this file): Smarty token grammar + content-reference forms + the silent-zero trap in `audience-query-mechanics.md` (see "Content references (for usage scans)"); the where-used traversal pattern, the message-list `updatedAt`-not-`sentAt` sort trap, and the suspicious-result law in `tool-ergonomics.md`; the **object graph** (content layer: messages → templates/blocks → includes → blockData; orchestration → message-node ids; journey-node messages invisible to `list_messages`) in `entity-model.md`. **Open and read the referenced `reference/` files before building any query — they are required, not optional (do not work from the SKILL body alone).** This recipe only adds the reverse-scan specifics.

## Steps (the linked path)

1. **Orient** — `whoami` to confirm the account.
2. **Resolve the target to an exact key/string — fail closed if it doesn't resolve.** Decide what the user means and pin the literal to grep for. **Handoff key: the literal content key OR attribute key OR coupon string.**
   - Sculpt block → `list_sculpt_blocks`, map the friendly name to its `key` (and `id` — `get_sculpt_block` takes **id only, no key arg**).
   - HTML include → `list_html_includes`, take `key` (`get_html_include` accepts the **key** directly).
   - Contact attribute → `describe_contact_schema`, take `key`; the literal in content is `{$contact.<key>}`.
   - Coupon code → just the literal string (e.g. `SAVE20`), or `{$contact.<key>}` if it's a personalized PIN/code attribute. Coupons are often **not** static literals — check `get_account_supplements` / `list_supplements` for a coupon table and any `crm_coupon*`-style attributes; the real "where used" may resolve to a supplement (`getSupplementRecords`) or a month-rotating include, not a `SAVE20` string.
   - Image → `search_images` for the key/URL.
   - **No exact match? STOP — this is a resolution failure, not an empty result.** If the user's token does not match any `key`/`id`/attribute after a **full enumeration**, do not proceed to scan and do **not** report "not used anywhere." Two distinct failure modes, each handled the same way: (a) the name maps to a **family** of keys (user says `footer`; account has `footer_standard` / `footer_minimal` / `footer_legal`) → return all near-matches and ask which; (b) the name **doesn't exist at all** (`hero_banner` with no such block; `{$contact.member_pin}` not in schema) → return resolution failure + fuzzy/substring near-matches on **both** name AND key, and offer to scan once disambiguated. A schema attribute that doesn't exist can only ever return 0 references — that 0 means "attribute doesn't exist," not "safe to change."
3. **Enumerate ALL candidate assets — both message types, every page; but check feasibility first.** `list_messages` defaults to `type:batch`; automated templates need an explicit `type:automated` — run **both** enumerations or you under-count. The list often sorts by `updatedAt`, paginates, and is **not** ordered by relevance — scan **every page**, never early-stop on page position. Also enumerate `list_orchestrations`, `list_sculpt_blocks`, `list_sculpt_templates`, `list_html_includes` (includes can include includes, so they're both targets and scan-candidates). **Handoff keys: message `id`, orchestration `id`, template/block/include keys.**
   - **Large-corpus escalation.** Read `totalEntries` first. Per-message `get_message` is infeasible at scale (real accounts hit 20k+ batch messages / hundreds of pages). When the message corpus is too large to deterministically scan in budget, **do not page through tens of thousands of bodies pretending at full enumeration.** Instead: (a) resolve the **binding architecture** first — templates + sculpt blocks + includes are the small, finite layer where references actually live; scan those exhaustively. (b) Targeted-scan only high-signal messages (name/tag matches the target). (c) Always report the message-body coverage gap explicitly rather than implying a full scan. "Scan every page" applies to the small enumerations (orchestrations, blocks, includes, templates); for a tens-of-thousands message corpus it yields to this escalation.
4. **Scan each candidate's body for the literal.** Fetch and string-search:
   - `get_message(messageId=<id>)` → scan `subject` AND `content.html`.
   - `get_html_include(key=<key>)` → scan `content` (and `wip` if relevant — state which version you scanned).
   - `get_sculpt_block(id=<id>)` → scan `html`. `get_sculpt_template` likewise.
   Match all reference forms: `content:<key>` and bare `<key>` (for includes), `{$contact.<key>}` (attributes), the literal coupon string, the image key. **Flag dynamic includes** (`{...|default:'header'}`, variable-driven) as **possible/conditional** — they can't be statically resolved to one key with certainty.
   - **The "select HTML block" binding trap (primary place dynamic references hide).** Many accounts bind includes through a reusable sculpt block whose html is `{include "content:{$blockData.<...>.select_block|default:'<fallback>'}"}` (a "select menu of all HTML Content" widget). The **actually-selected** include key is stored in each message's per-message `blockData` and is **NOT** present in the message HTML — `get_message` exposes only the `|default` fallback. So a static body-grep systematically **misses** the real binding and will falsely report "not used." When you see this pattern, treat every message built on that block as a **possible** consumer of your target, name the binding block as the locus, and state plainly that the selection is not API-exposed (a confirmed-unidentifiable slice). Read the binding block + a representative template instead of mass-grepping bodies.
5. **Expand orchestrations to their message nodes.** `get_orchestration(id=<id>)` → `composition.actions[]` where `settings.action.type === "Message"`; the node's `settings.action.id` is the message `id` (note: `settings.action.publishedId` is a separate content hash, NOT the message id). `get_message` each node and scan. A deep journey can be many `get_message` calls; decide whether `DISABLED___` / retired orchestrations count as "in use" and say which.
6. **Data-job signal — job level only.** `list_data_automations` + `get_data_automation(id)` expose `name`/`key`/`tags` + trigger `filter`/`filterTranslated` + schedule — a *usage signal* only. The **inner transformation script is NOT exposed** (honest limit), so a job's script may reference the target invisibly. Emit this as an explicit **"unidentified — data-job transform scripts not exposed"** slice. Then check `list_data_batches` / `get_data_batch` for a one-time batch whose `transform.dataTransform` may leak the full Smarty and upgrade an unidentified hit to confirmed.
7. **Compile the where-used report.** List each referencing asset by type + name/key, mark dynamic-include matches as "possible," and carry the explicit unidentified data-job slice. If the asset budget can't deterministically cover the account, **report the coverage gap** rather than a false "not used anywhere."

## Output

A where-used report (chat — list/table), grouped by asset type, where each row is the referencing asset's type + name/key (+ message/orchestration id). Sections: **confirmed references** | **possible/conditional** (dynamic includes + "select HTML block" blockData consumers) | **unidentified — not exposed by the API** (data-job transform scripts; per-message `blockData` include selections) | **coverage note** (which message types + how many pages scanned; any large-corpus gap). When the target spans message classes (e.g. a marketing footer vs a `footer_legal` footer), **segment by target key + message classification** — retiring one footer key does not touch sends that use a different one. State which content version you scanned (`content` vs `wip`; published vs draft composition). All reads are read-only — never imply the target or any referencing asset was changed.

## Guardrails

- **Read-only.** A where-used scan reads content; never imply the target was edited, deleted, or that anything was saved.
- **Both message types, all pages.** `type:batch` and `type:automated` are separate enumerations; the list sorts by `updatedAt` not relevance — scan every page, never early-stop on page position (see tool-ergonomics). Under-scanning produces a false "not used."
- **No reverse index exists.** Coverage = enumerate-and-scan; if the budget can't cover the account deterministically, report the gap, don't assert "not used anywhere."
- **Resolve-or-stop.** Step 2 must pin an exact `key`/`id`/attribute. If it doesn't after a full enumeration, return a **resolution failure + near-matches** (family or fuzzy), never a scan for an invented literal and never "not used anywhere." A 0 on a target that doesn't exist (e.g. a schema attribute absent from `describe_contact_schema`) means "doesn't exist," not "safe to change."
- **Two invisible slices, not one.** (a) Data-job inner transformation scripts — report at job level (name/key/tags/filter only); check one-time batches for a leaked transform before settling. (b) Per-message `blockData` include selections behind a "select HTML block" — `get_message` shows only the `|default` fallback, so the chosen key is unverifiable from the API. Both are confirmed-unidentifiable; never infer their contents and never let either become a false "safe to retire."
- **Reference forms vary** (`{$contact.<key>}`, `{include 'content:<key>'}`, literal coupon strings, image keys, indirect supplement references). Note the source type so attribution is accurate; flag dynamic/variable-driven includes as possible, not confirmed.
- **Block ergonomics mismatch:** `get_sculpt_block` is id-only (resolve key→id via `list_sculpt_blocks` first); `get_html_include` accepts key directly. Orchestrations don't inline message content — always expand to message nodes.
- If a result looks suspicious (0 references on an obviously-used asset, too clean), apply the suspicious-result law (tool-ergonomics) before reporting — re-check the literal, casing, and reference form. **This also covers "the asset name itself isn't found":** before concluding non-existence, verify casing and hyphen-vs-underscore vs synonyms (`hero_banner` vs `app-banner`; `footer` vs `footer_standard`) across the full enumeration — a miss is often a naming mismatch, not a missing asset.

## Worked example (illustrative — fictional values)

A marketer is about to retire the HTML include `footer_standard` and asks what breaks.

1. `whoami` → confirm account.
2. `list_html_includes` → the user said `footer`, but no key is exactly `footer`; the account has a **family** (`footer_standard` / `footer_minimal` / `footer_legal`). Disambiguate → user means the marketing `footer_standard`. Literals to grep: `content:footer_standard`, bare `footer_standard`, and `default:'footer_standard'`.
3. Enumerate: `list_messages type:automated` (all pages) + `list_messages type:batch` — **check `totalEntries` first.** If batch is tens of thousands of messages, declare the body corpus unscannable up front and pivot to the binding layer (next step) + targeted high-signal scans + an explicit coverage gap. `list_orchestrations` (all pages) + `list_sculpt_blocks` + `list_html_includes` + `list_sculpt_templates` are small — scan exhaustively.
4. Resolve the **binding architecture** in the small finite layer: a template (e.g. the marketing layout) statically includes `{include 'content:footer_standard'}` → **confirmed**. But marketing messages may bind the footer dynamically through a "select HTML block" sculpt block, with the chosen key in per-message `blockData` (invisible to `get_message`) → every such message is a **possible** consumer, reported as an API-unexposed slice. A `|default:'footer_standard'` reference is **possible/conditional**. Transactional sends use a different key (`footer_legal`) → segment them out.
5. For each include/block/template: fetch and grep — say 2 other includes pull `content:footer_standard` (includes including includes).
6. Orchestrations: `get_orchestration` → for `composition.actions[]` with `settings.action.type==='Message'`, `get_message(settings.action.id)` and grep; suppose 4 journeys reference it across their nodes.
7. Data jobs: `list_data_automations` + `get_data_automation` → no filter signal; emit **"unidentified — data-job transform scripts not exposed (N jobs unscanned)"**; `list_data_batches` had no recent one-time batch leaking a transform.
8. **Compile:** confirmed references (the static template + includes + journeys) | possible/conditional (the dynamic default + every "select HTML block" message whose `blockData` selection isn't API-exposed) | unidentified (data-job transform scripts; per-message blockData selections) | coverage note (scanned the small finite binding layer exhaustively; targeted-scanned high-signal messages; if batch was tens of thousands, body coverage is a stated gap). Verdict: safe to retire **only** after updating the confirmed references and manually checking the possible/unidentified slices — a clean static grep alone would have falsely said "not used" (read-only finding — nothing changed).
